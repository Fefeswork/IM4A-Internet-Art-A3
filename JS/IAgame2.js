//sancho panza	
     ...
     charCtx.clearRect(0, 0, 60, 60);char = new NewChar();
        ...
        function fiftyShadesOfHeroes(x){
            
            for(let n = 0;n < x;n++){
        
        var gender = ["male","female","nonbinary"];
       gender = gender[Math.floor((Math.random() * 3))]; 
        console.log(gender);
            
        var race = [0,90,180,300];
            race = race[Math.floor((Math.random() * 4))]; 
        console.log(race);
            
            var ethn = [0.6,0.7,0.8,0.9,1,1.1,1.2];
            ethn = ethn[Math.floor((Math.random() * 6))]; 
        console.log(ethn);
            
            var hair =  [ [Math.floor((Math.random() * 360)+1)],[Math.floor((Math.random() * 4))],[48, 240, 336] ];
            console.log(hair);
            
            var eyes = Math.floor((Math.random() * 360)+1);
            
            char = new NewChar(gender,race,ethn,hair,eyes);
                if(x>1)char.giveBirth();
            };
    
      }
     fiftyShadesOfHeroes(25);
